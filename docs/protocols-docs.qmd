...
protocols_df_neuro_mr_spine <- protocols_df |> 
  filter(exam_name %in% "MRI Screening Whole Spine")
...
```{r protocols_df_mr_spine_tbl, echo=FALSE}
protocols_df_neuro_mr_spine %>% 
  gt_custom() |> 
  cols_label(exam_name = "Exam") |> 
  opt_stylize(style = 3, color = "cyan")
```
